...
function DokumentSide(props: Props) {
  ...
  const bildetekst = intl.formatMessage(
    { id: "dokumentinnsyn.side.alttekst" },
    { sidetall: props.side, tittel }
  );
  return props.kanVises ? (
    robustImg(
      props.url,
      { alt: bildetekst, tabIndex: "0" },
      renderFeilkomponent(props.openPdfUrl)
    )
  ) : (
    <DokumentFeilmelding
      url={props.url}
      feilmelding={props.feilmelding}
      ekstrafeilinfo={props.ekstrafeilinfo}
    />
  );
}
