...
import {
  harTilgangTilKommunaleTemagrupper,
  TilgangState,
} from "../ducks/tilgang";
...
const ukjentTemagruppeTittel = (
  <FormattedMessage id="skriv-sporsmal.ukjent-temagruppe" />
);
...
function SkrivNyttSporsmal(props: Props) {
  ...
  if (temagruppe.toLowerCase() === "oksos") {
    if (props.tilgang.status === STATUS.PENDING) {
      ...
      props.tilgang.status === STATUS.OK &&
      props.tilgang.data.resultat !== "OK"
    ) {
      return (
        <Alertstripe type="info">
          <FormattedMessage
            id={`feilmelding.kommunalsjekk.${props.tilgang.data.resultat}`}
          />
        </Alertstripe>
      );
    }
  ...
  return (
    <article className="blokk-center send-sporsmal-side skriv-nytt-sporsmal">
      ...
      <form className="panel text-center" onSubmit={submit}>
        ...
        <AlertstripeVisibleIf
          type="advarsel"
          visibleIf={props.sendingStatus === STATUS.ERROR}
        >
...
const mapStateToProps = ({ ledetekster, traader, ui, tilgang }: AppState) => ({
  ...
  godkjenteTemagrupper: harData(ledetekster)
    ? ledetekster.godkjenteTemagrupper
    : [],
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(SkrivNyttSporsmal)
);
