...
const hentDokumentMetadata = (journalpostId: string, dokumentmetadata: string) =>
    fetchToJson(`${API_BASE_URL}/dokumenter/dokumentmetadata/${journalpostId}/${dokumentmetadata}`, MED_CREDENTIALS);
...
const hentAlleDokumentMetadata = (journalpostId: string, dokumentreferanser: string) =>
    Promise.all(dokumentreferanser.split('-').map((dokumentId) => hentDokumentMetadata(journalpostId, dokumentId)));
