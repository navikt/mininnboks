...
type DokumentvisningDataOk = Action<TypeKeys.DOKUMENTVISNING_DATA_OK> &
  DucksData<any>;
...
type StatusPdfModal = Action<TypeKeys.STATUS_PDF_MODAL> & {
  pdfModal: PdfModal;
};

type Actions =
  | DokumentvisningDataOk
  | DokumentvisningDataFeilet
  | DokumentvisningDataPending
  | StatusPdfModal;
  ...
  switch (action.type) {
    ...
    case TypeKeys.DOKUMENTVISNING_DATA_OK: {
      ...
      return {
        ...state,
        status: STATUS.OK,
        data: { dokumentmetadata, journalpostmetadata },
      };
    }
...
// ActionCreators
const hentDokumentMetadata = (
  journalpostId: string,
  dokumentmetadata: string
) =>
  fetchToJson(
    `${API_BASE_URL}/dokumenter/dokumentmetadata/${journalpostId}/${dokumentmetadata}`,
    MED_CREDENTIALS
  );

const hentJournalpostMetadata = (journalpostId: string) =>
  fetchToJson(
    `${API_BASE_URL}/dokumenter/journalpostmetadata/${journalpostId}`,
    MED_CREDENTIALS
  );

const hentAlleDokumentMetadata = (
  journalpostId: string,
  dokumentreferanser: string
) =>
  Promise.all(
    dokumentreferanser
      .split("-")
      .map((dokumentId) => hentDokumentMetadata(journalpostId, dokumentId))
  );

export function hentDokumentVisningData(
  journalpostId: string,
  dokumentreferanser: string
) {
