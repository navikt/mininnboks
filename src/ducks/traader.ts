...
type MarkertSomLestOk = Action<TypeKeys.MARKERT_SOM_LEST_OK> &
  DucksData<{ traadId: string }>;
type MarkertSomLestFeilet = Action<TypeKeys.MARKERT_SOM_LEST_FEILET> &
  DucksData<Error>;
...
export default function reducer(
  state: TraaderState = initalState,
  action: Actions
): TraaderState {
  switch (action.type) {
...
export const sendSvar = (traadId: string, fritekst: string) => (
  dispatch: Dispatch<any>
) =>
  doThenDispatch(
    () =>
      Api.sendSvar(traadId, fritekst).then(() =>
        dispatch(hentTraader(TypeKeys.HENT_ALLE_RELOAD))
      ),
    innsendingActions
  )(dispatch);
...
function flettDelviseSvarInnISkriftligSvar(
  traad: Traad,
  delviseSvar: Melding[]
) {
...
function flettMeldingerITraad(traad: Traad): Traad {
  ...
  sammenslaatTraad.meldinger = traad.meldinger.filter(
    (melding) => !erDelvisSvar(melding)
  );
  return sammenslaatTraad;
}

export function selectTraaderMedSammenslatteMeldinger(
  store: AppState
): { data: Traad[] } {
