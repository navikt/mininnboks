import reducer, {
  selectTraaderMedSammenslatteMeldinger,
  TraaderState,
  TypeKeys,
} from "./traader";
...
describe("traader-ducks", () => {
  describe("reducer", () => {
    it("skal oppdatere alle meldinger med med rett traadId med status lest", () => {
      ...
      function erAlleMeldingerLest(traadId: string) {
        const traader =
          markertSomLest.status === STATUS.OK ? markertSomLest.data : [];
        return traader
  ...
  describe("selector", () => {
    describe("traaderMedSammenslatteMeldinger", () => {
      ...
      const SVAR_TEKST =
        "Jeg avslutter det delvise spørsmålet ved å svare på oppgaven";
      ...
      it("skal ikke selecte delvise svar", () => {
        const sammenslaatteTraader = selectTraaderMedSammenslatteMeldinger(
          initialState
        );
